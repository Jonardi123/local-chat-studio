...
export type AgentName='grace'|'ada'|'knuth'|'margaret'|'linus'|'turing'
...
export interface Checkpoint{summary:string;throughMessageId:string;estimatedTokens:number;updatedAt:number}
export interface Conversation{id:string;title:string;messages:Message[];createdAt:number;updatedAt:number;parentChatId?:string;parentMessageId?:string;checkpoint?:Checkpoint}
export interface Settings{baseUrl:string;apiKey:string;model:string;temperature:number;maxTokens:number;systemPrompt:string;theme:Theme;reasoningMode:ReasoningMode}
export interface AppData{version:2;settings:Settings;conversations:Conversation[];activeId:string|null;onboarded:boolean}
