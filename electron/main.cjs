...
async function unusedDownloadPath(name){const parsed=path.parse(name),dir=app.getPath('downloads');for(let i=0;;i+=1){const file=path.join(dir,`${parsed.name}${i?` (${i})`:''}${parsed.ext}`);try{await fs.access(file)}catch{return file}}}
ipcMain.handle('update:check',updateInfo)
ipcMain.handle('update:download',async event=>{const release=await latestRelease(),asset=selectAsset(release,process.platform),expected=/^sha256:([a-f0-9]{64})$/i.exec(String(asset?.digest??''))?.[1];if(!asset||!expected||!isNewer(release.tag_name,app.getVersion()))throw new Error('No verified compatible update is available.');const response=await net.fetch(asset.browser_download_url,{headers:{'User-Agent':'sudoN-updater'}});if(!response.ok||!response.body)throw new Error(`Update download returned HTTP ${response.status}.`);const destination=await unusedDownloadPath(path.basename(asset.name)),temporary=`${destination}.download`,total=Number(response.headers.get('content-length'))||Number(asset.size)||0,hash=createHash('sha256');let received=0;const source=Readable.fromWeb(response.body).on('data',chunk=>{hash.update(chunk);received+=chunk.length;event.sender.send('update:progress',{received,total,percent:total?Math.round(received/total*100):null})});try{await pipeline(source,fsNative.createWriteStream(temporary,{flags:'wx'}));if(hash.digest('hex')!==expected.toLowerCase())throw new Error('The downloaded update failed its SHA-256 integrity check.');await fs.rename(temporary,destination)}catch(error){await fs.rm(temporary,{force:true});throw error}const openError=await shell.openPath(destination);if(openError)throw new Error(openError);return{path:destination}})
